...
public class DataSeeder {
    ...
    CommandLineRunner initDatabase(UserRepository userRepository,
        ...
        return args -> {
            ...
            // ================= ADMIN ACCOUNTS =================
            createAdminAccount(userRepository, passwordEncoder, "admin@cyclemart.com", "Admin CycleMart", "0901000001",
                    "admin123");
            createAdminAccount(userRepository, passwordEncoder, "admin2@cyclemart.com", "Admin 2 CycleMart",
                    "0901000002", "admin123");

            // ================= USER ACCOUNTS =================
            createUserAccount(userRepository, passwordEncoder, "user1@test.com", "Nguyễn Văn A", "0912345001",
                    "31072005Xy09@");
            createUserAccount(userRepository, passwordEncoder, "user2@test.com", "Trần Thị B", "0912345002",
                    "31072005Xy09@");

            // ================= INSPECTOR ACCOUNTS =================
            createInspectorAccount(userRepository, passwordEncoder, "inspector@test.com", "Trần Kiểm Định (Ảo 1)",
                    "0912345678", "inspector123");
            createInspectorAccount(userRepository, passwordEncoder, "inspector2@test.com", "Nguyễn Kiểm Định (Ảo 2)",
                    "0987654321", "inspector123");
            createInspectorAccount(userRepository, passwordEncoder, "inspector3@test.com", "Lê Kiểm Định (Ảo 3)",
                    "0987654322", "inspector123");
    ...
    private void seedCategories(CategoryRepository categoryRepository) {
        if (categoryRepository.count() == 0) {
            ...
            Categories sportBikes = createCategory("Xe đạp thể thao",
                    "Các dòng xe chuyên dụng cho tập luyện, đua và vượt địa hình", null, 1);
            Categories casualBikes = createCategory("Xe đạp phổ thông", "Xe đạp đi lại hàng ngày trong thành phố", null,
                    2);
            ...
            Categories roadBike = createCategory("Xe đạp đua (Road)", "Tối ưu cho đường nhựa, tốc độ cao", sportBikes,
                    1);
            Categories mtb = createCategory("Xe đạp địa hình (MTB)", "Giảm xóc tốt, lốp bám đường cho đồi núi",
                    sportBikes, 2);
            ...
            Categories cityBike = createCategory("Xe đạp thành phố (City)", "Dáng ngồi thoải mái, có giỏ/baga",
                    casualBikes, 1);
            Categories kidsBike = createCategory("Xe đạp trẻ em", "Kích thước nhỏ gọn, an toàn cho bé", casualBikes, 2);
            Categories foldingBike = createCategory("Xe đạp gấp", "Dễ dàng gấp gọn mang lên ô tô, tàu điện",
                    casualBikes, 3);

            Categories groupset = createCategory("Bộ truyền động (Groupset)", "Đề, líp, đùi đĩa Shimano, SRAM...",
                    parts, 1);
    ...
    private void seedInspectionCriteria(InspectionCriterionRepository repository) {
        if (repository.count() == 0) {
            ...
            List<InspectionCriterion> criteria = List.of(
                    createCriterion("Khung sườn & Sơn",
                            "Kiểm tra nứt gãy, cong vênh, rỉ sét, móp méo, tình trạng lớp sơn zin/sơn lại."),
                    createCriterion("Hệ thống phanh (Thắng)",
                            "Kiểm tra độ mòn má phanh, dây phanh, đĩa/niềng phanh, tay phanh, độ nhạy và an toàn."),
                    createCriterion("Hệ thống truyền động (Groupset)",
                            "Kiểm tra xích, líp, đĩa, củ đề trước/sau, tay đề, dây đề, độ mượt khi sang số."),
                    createCriterion("Bánh xe & Lốp",
                            "Kiểm tra vành (niềng) cong vênh, căm lỏng/gãy, lốp xe (mòn, rách), ruột xe, áp suất lốp."),
                    createCriterion("Hệ thống lái & Yên xe",
                            "Kiểm tra ghi đông, pô tăng, chén cổ (cổ phốt), cốt yên, yên xe có lỏng lẻo hay nứt gãy không."),
                    createCriterion("Phuộc / Giảm xóc (Nếu có)",
                            "Kiểm tra độ nhún, xì dầu, nứt gãy, hoạt động của khóa phuộc (Lockout)."));
